...
{
    const std::string URL_SOURCE_CODE = "<https://www.notebc.io/>"; // need to change once get the git repo details & url 
    const std::string URL_WEBSITE = "<https://www.notebc.com/>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if notecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if notecoin is started while in a temporary directory.\n",
